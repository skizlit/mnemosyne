...
def test_public_placeholder_passes(tmp_path: Path) -> None:
...
def test_prohibited_vault_path_fails(tmp_path: Path) -> None:
    root = make_root(tmp_path)
...
def test_synthetic_private_config_fails_without_echoing_value(tmp_path: Path) -> None:
    root = make_root(tmp_path)
